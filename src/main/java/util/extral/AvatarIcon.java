...
public class AvatarIcon implements Icon {
  ...
  private void paintBorder(Graphics2D g, Shape mask, int x, int y, int width, int height,
      int borderSize, int innerSize, float round) {
    Area areaBorder = new Area(createMask(width, height, 0, round));
    Shape shape = innerSize > 0
        ? createMask(width - borderSize * 2, height - borderSize * 2, borderSize, round)
        : mask;
    if (shape != null) {
  ...
  public static class BorderColor {
    ...
    public void paint(Graphics2D g, int x, int y, int width, int height) {
      ...
      if (endColor == null) {
        ...
        g.setPaint(new GradientPaint(x, y + height * startPoint, startColor, x + width,
            y + height * endPoint, endColor));
      }
    }
