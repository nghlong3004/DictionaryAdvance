...
import util.view.ShadowRenderer;
...
public class DropShadowBorder extends EmptyBorder {
   ...
   * 
   */
  private static final long serialVersionUID = -7406555391620094693L;
  @Styleable
  ...
  public void paintBorder(Component c, Graphics g, int x, int y, int width, int height) {
    ...
    if (shadowImage == null || !shadowColor.equals(lastShadowColor) || width != lastWidth
        || height != lastHeight || shadowSize != lastShadowSize
        || shadowOpacity != lastShadowOpacity || arc != lastArc) {
  ...
  private void createEffect(JComponent c, Graphics2D g2, int x, int y, int width, int height,
      int arc) {
    ...
    if (!effectRight) {
      g2.setPaint(
          new GradientPaint(x, 0, effectColor, x + (width * effectWidth), 0, c.getBackground()));
    } else {
  ...
  private BufferedImage createShadowImage(int width, int height, int arc) {
    ...
    Shape shape = FlatUIUtils.createRoundRectanglePath(0, 0, shadowWidth, shadowHeight, round,
        round, round, round);
