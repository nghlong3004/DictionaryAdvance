...
import static view.notifications.ToastClientProperties.*;
...
public class ToastPanelUI extends BasicPanelUI implements StyleableUI, PropertyChangeListener {
  ...
  protected void installDefaults(JPanel p) {
    ...
    margin = NotificationUI.getInsets(prefix + ".margin", new Insets(8, 8, 8, 8));
    ...
    closeButtonIcon = NotificationUI.getIcon(prefix + ".closeIcon",
        NotificationUI.createIcon("image/notification/close.svg", closeIconColor, 0.75f));
    ...
    effectAlignment = NotificationUI.getString(prefix + ".effectAlignment", "left");
  ...
  protected Border createDefaultBorder() {
    ...
    Insets insets = NotificationUI.getInsets("Toast.shadowInsets", new Insets(0, 0, 6, 6));
      ...
      synchronized (parent.getTreeLock()) {
        ...
        if (component != null) {
          int cW = maximumWidth == -1 ? component.getPreferredSize().width
              : Math.min(component.getPreferredSize().width,
